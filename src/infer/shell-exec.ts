...
import { CORE } from "./coreutils.ts";
import type { ShellState } from "./coreutils.ts";

export interface ExecResult { out: string; exit: number; dreamed?: string; }

// split on top-level separators (outside quotes)
function splitTop(s: string, seps: string[]): { part: string; sep: string }[] {
  const out: { part: string; sep: string }[] = [];
  let buf = "", q: string | null = null;
  for (let i = 0; i < s.length; i++) {
    ...
    if (sep) { out.push({ part: buf, sep }); buf = ""; i += sep.length - 1; continue; }
    buf += c;
  }
...
function tokenize(s: string): string[] {
  const toks: string[] = [];
  let buf = "", q: string | null = null, has = false;
  for (let i = 0; i < s.length; i++) {
    const c = s[i];
    if (q) { if (c === q) q = null; else buf += c; has = true; continue; }
    if (c === "'" || c === '"') { q = c; has = true; continue; }
    if (c === " " || c === "\t") { if (has) { toks.push(buf); buf = ""; has = false; } continue; }
    buf += c; has = true;
  }
...
function expand(tok: string, st: ShellState, quotedDouble: boolean): string[] {
  let t = tok;
  t = t.replace(/\$\?/g, String(st.lastExit));
  t = t.replace(/\$\{(\w+)\}|\$(\w+)/g, (_m, a, b) => st.env.get(a ?? b) ?? "");
  if (t === "~" || t.startsWith("~/")) t = (st.user === "root" ? "/root" : "/home/guest") + t.slice(1);
...
function runPipeline(pipeline: string, st: ShellState): ExecResult | null {
  ...
  if (stages.length === 1) {
    ...
    if (m && !/\s/.test(m[1])) { st.env.set(m[1], stripQuotes(m[2])); return { out: "", exit: 0 }; }
  }
  // parse each stage into { cmd, args, redir }
  const parsed = stages.map((stage) => {
    const raw = tokenize(stage);
    const words: string[] = [];
    let redir: { op: string; file: string } | null = null;
    for (let i = 0; i < raw.length; i++) {
      if (raw[i] === ">" || raw[i] === ">>") { redir = { op: raw[i], file: raw[i + 1] }; i++; continue; }
      const m = raw[i].match(/^(>>?)(.+)$/);
      if (m) { redir = { op: m[1], file: m[2] }; continue; }
      words.push(raw[i]);
    }
  ...
  if (parsed.some((p) => !(p.cmd in CORE))) return null;
...
const isAssign = (seg: string) => /^\w+=/.test(seg) && !/^\w+=\S*\s/.test(seg.replace(/^\w+=/, "x="));

export function execLine(line: string, st: ShellState): ExecResult {
  const trimmed = line.trim();
  if (!trimmed) return { out: "", exit: st.lastExit };
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    if ((prevSep === "&&" && exit !== 0) || (prevSep === "||" && exit === 0)) continue;
    const res = runPipeline(seg, st);
    if (res === null) return { out, exit, dreamed: trimmed };
    out += res.out;
    exit = res.exit;
  }
  return { out, exit };
