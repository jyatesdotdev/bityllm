...
import { deserialize, InferenceSession, GPUInferenceSession, Shell, BINARIES, completeCommand, History } from "../../src/infer.ts";
...
// fetch + deserialize a model file → an active session (WebGPU vs CPU raced) + metadata
type Loaded = { session: SessionLike; engine: string; model: ReturnType<typeof deserialize>["model"]; step: number; kb: number };
async function loadModel(file: string): Promise<Loaded | null> {
  let res = await fetch(file);
  if (!res.ok) res = await fetch("/models/" + file); // repo-root fallback (local dev)
  ...
  const warm = async (s: SessionLike): Promise<void> => {
    s.feed("guest@bity:~$ ls\n");
  ...
  try {
    ...
    if (gRate > cRate) { session = gpuS; engine = `webgpu ${gRate.toFixed(0)} tok/s (cpu ${cRate.toFixed(0)})`; }
    else { gpuS.destroy(); engine = `cpu ${cRate.toFixed(0)} tok/s (webgpu ${gRate.toFixed(0)})`; }
  } catch { /* no WebGPU in this browser — CPU it is */ }
  return { session, engine, model, step, kb: Math.round(buf.length / 1024) };
}

const banner = (m: (typeof MODELS)[number], l: Loaded): string =>
  `bity 0.1 · model: ${m.label} (${m.note} params, ${m.ver}, step ${l.step}) · ${l.kb} KB int8 · engine: ${l.engine}\n` +
...
async function main(): Promise<void> {
  ...
  if (!loaded && modelIdx !== DEFAULT_MODEL) { modelIdx = DEFAULT_MODEL; loaded = await loadModel(MODELS[modelIdx].file); }
  clearInterval(tick);
  if (!loaded) {
  ...
  const computeGhost = (): void => {
    if (!suggestOn || !powered || busy || line.length === 0) return;
    if (peekInFlight) return; // the in-flight peek re-checks freshness when done
    if (session.length + line.length + 40 >= model.cfg.blockSize) return; // rewind hazard
    const gen = ++suggestGen;
    const forLine = line;
    peekInFlight = true;
    void withLock(async () => {
      try {
        if (gen !== suggestGen || busy || !powered) return;
        ...
        try {
          ...
          out = out.split("\n")[0];
          if (gen === suggestGen && !busy && powered && out.length > 0) setGhost(out);
        } finally {
  ...
  const scheduleGhost = (): void => {
    ...
    suggestGen++;
    clearTimeout(suggestTimer);
    if (suggestOn) suggestTimer = setTimeout(computeGhost, 220);
  };
  ...
  let switching = false;
  async function switchModel(idx: number): Promise<void> {
    if (idx === modelIdx || switching) return;
    switching = true;
    await withLock(async () => {
      ...
      (shell.session as { destroy?: () => void }).destroy?.(); // free the old GPU session
  ...
  const knob = (id: string, positions: KnobPos[], start: number, persist?: string): void => {
    const el = document.getElementById(id)!;
    const dial = el.querySelector(".dial") as HTMLElement;
    const val = el.querySelector(".kval") as HTMLElement;
    let idx = start;
    if (persist) {
      const saved = parseInt(localStorage.getItem(persist) ?? "", 10);
      if (saved >= 0 && saved < positions.length) idx = saved;
    }
    const render = (): void => {
    ...
    el.addEventListener("click", () => {
      ...
      if (persist) localStorage.setItem(persist, String(idx));
      render();
    });
    render();
  };
  ...
  document.addEventListener("keydown", async (e: KeyboardEvent) => {
    if (!powered || busy || e.metaKey || e.ctrlKey || e.altKey) return;
