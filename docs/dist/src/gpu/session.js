...
import * as KI from "./wgsl-infer.js";
import * as KF from "./wgsl-fused.js";
import { LN_FWD, GELU_FWD } from "./wgsl.js";
export class GPUInferenceSession {
    ...
    curT = 0;
    last = null;
    dirty = false; // tokens submitted since the last logits readback
    constructor(device, model, tok) {
        this.device = device;
        this.model = model;
        this.tok = tok;
    }
    static async create(model, tok) {
        if (typeof navigator === "undefined" || !navigator.gpu)
            throw new Error("no WebGPU in this runtime");
        ...
        if (err)
            throw new Error(`GPU session build failed validation: ${err.message}`);
        return s;
    }
    // ---- construction ----------------------------------------------------------
    pipe(name, code) {
    ...
    buf(n, usage = GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST | GPUBufferUsage.COPY_SRC) {
    ...
    op(name, code, uniFields, buffers, threads, perWorkgroup = 64) {
    ...
    build() {
        ...
        for (const [name, t] of this.model.namedParameters()) {
            ...
            dev.queue.writeBuffer(b, 0, t.data.buffer, t.data.byteOffset, t.size * 4);
            W.set(name, b);
        }
        const w = (n) => W.get(n);
        ...
        const gemv = (X, wgt, Y, K, N, bias, atPos = false) => this.op("gemv", KI.GEMV, [["u", K], ["u", N], ["u", bias ? 1 : 0], ["u", atPos ? 1 : 0]], [this.globals, X, wgt, Y, bias ?? m1], N);
        const ln = (X, wn, bn, Y) => this.op("ln", LN_FWD, [["u", C], ["u", 1]], [X, w(wn), w(bn), Y, m1, r1], 128, 128);
        const accum = (A, Cc) => this.op("accum", KI.ACCUM, [["u", C]], [A, Cc], C, 256);
        ...
        for (let l = 0; l < L; l++) {
            ...
            this.op("attn", KI.ATTN_ONE, [["u", C], ["u", nH], ["u", hd], ["f", 1 / Math.sqrt(hd)]], [this.globals, qRow, Kc[l], Vc[l], attnO], nH * 128, 128);
        ...
        this.op("gemv_t", KI.GEMV_T, [["u", C], ["u", V], ["u", 0], ["u", 0]], [lnO, w("wte"), this.logitsBuf], V);
    ...
    async generate(prompt, opts) {
        this.feed(prompt);
        let s = "";
        for await (const ch of this.stream(opts))
            s += ch;
        return s;
    }
    /** Current next-token logits (readback). Exposed for parity testing. */
    async logits() {
        if (this.dirty || this.last === null)
            this.last = await this.readLogits();
        return this.last;
    }
    // ---- internals --------------------------------------------------------------
    push(id) {
