...
import { Linear, LayerNorm, MLP } from "./layers.js";
...
export class GPT extends Module {
    ...
    forward(idx, B, T) {
        if (T > this.cfg.blockSize)
            throw new Error(`T=${T} exceeds blockSize=${this.cfg.blockSize}`);
        ...
        if (p > 0 && this.training)
            x = ops.dropout(x, p, this.dropRng);
    ...
    paramGroups() {
        const decay = [];
        const noDecay = [this.wte, this.wpe];
        for (const b of this.blocks) {
