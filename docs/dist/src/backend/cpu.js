...
export class CPUBackend {
    ...
    from(values, shape) {
        if (values.length !== prod(shape))
            throw new Error(`from: ${values.length} values for shape [${shape}]`);
    ...
    matmul(a, b, opt = {}) {
        ...
        if (as.length < 2 || bs.length < 2)
            throw new Error("matmul: operands must be ≥2-D");
        ...
        if (K !== Kb)
            throw new Error(`matmul: inner dims ${K}≠${Kb} (a=[${as}] b=[${bs}] tA=${tA} tB=${tB})`);
    ...
    zipSuffix(a, b, f) {
        ...
        if (an % bn !== 0)
            throw new Error(`broadcast: |b|=${bn} does not divide |a|=${an}`);
    ...
    layerNorm(x, w, b, eps) {
        ...
        for (let r = 0; r < R; r++) {
            ...
            rstd[r] = rs;
            for (let j = 0; j < C; j++)
                y[o + j] = (xd[o + j] - mu) * rs * wd[j] + bd[j];
        }
        return { y: { data: y, shape: [...x.shape] }, mean, rstd };
    }
    layerNormBackward(x, w, mean, rstd, g) {
        const C = x.shape[x.shape.length - 1];
        const R = x.data.length / C;
    ...
    scatterAddRows(gradTable, cols, ids, g) {
        for (let i = 0; i < ids.length; i++) {
            const dst = ids[i] * cols, src = i * cols;
            for (let j = 0; j < cols; j++)
                gradTable[dst + j] += g[src + j];
        }
    }
    transpose12(a) {
        const [d0, d1, d2, d3] = a.shape;
        if (a.shape.length !== 4)
            throw new Error(`transpose12: expected 4-D, got [${a.shape}]`);
        const ad = a.data, out = this.alloc(ad.length);
    ...
    ceBackward(logits, lse, targets, gscale) {
        const [N, V] = logits.shape;
        ...
        for (let i = 0; i < N; i++) {
            const o = i * V, l = lse[i];
            for (let j = 0; j < V; j++)
                out[o + j] = Math.exp(zd[o + j] - l) * s; // softmax(z) · s
