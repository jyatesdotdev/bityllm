...
export const mkFile = (content = "", owner = "guest", mode = 0o644) => ({ kind: "file", content, mode, owner, group: owner, mtime: MT });
export const mkDir = (owner = "guest", mode = 0o755) => ({ kind: "dir", children: new Map(), mode, owner, group: owner, mtime: MT });
export class VFS {
    root = mkDir("root");
    ...
    isDir(path, cwd) { return this.lookup(path, cwd)?.kind === "dir"; }
    exists(path, cwd) { return this.lookup(path, cwd) !== null; }
    /** Sorted child names of a directory (optionally including dotfiles). */
    list(path, cwd, all = false) {
        const node = this.lookup(path, cwd);
        if (!node || node.kind !== "dir")
            return null;
        return [...node.children.keys()].filter((n) => all || !n.startsWith(".")).sort();
    }
    mkdir(path, cwd, parents = false, owner = "guest") {
        if (parents) {
            let acc = "/";
            for (const seg of this.segs(path, cwd)) {
                acc = acc === "/" ? "/" + seg : acc + "/" + seg;
                ...
                if (!at) {
                    const p = this.parent(acc, cwd);
                    p.dir?.children.set(p.name, mkDir(owner));
                }
                else if (at.kind !== "dir")
                    return { ok: false, err: `cannot create directory '${path}': Not a directory` };
            }
            return { ok: true };
        }
        const { dir, name } = this.parent(path, cwd);
        if (!dir)
            return { ok: false, err: `cannot create directory '${path}': No such file or directory` };
        if (dir.children.has(name))
            return { ok: false, err: `cannot create directory '${path}': File exists` };
        dir.children.set(name, mkDir(owner));
        return { ok: true };
    }
    writeFile(path, cwd, content, owner = "guest", append = false) {
        const { dir, name } = this.parent(path, cwd);
        if (!dir)
            return { ok: false, err: `cannot create '${path}': No such file or directory` };
        const existing = dir.children.get(name);
        if (existing?.kind === "dir")
            return { ok: false, err: `${path}: Is a directory` };
        if (append && existing?.kind === "file")
            existing.content += content;
        else
            dir.children.set(name, mkFile(content, owner));
        return { ok: true };
    }
    touch(path, cwd, owner = "guest") {
    ...
    remove(path, cwd, recursive = false) {
        const node = this.lookup(path, cwd);
        if (!node)
            return { ok: false, err: `cannot remove '${path}': No such file or directory` };
        if (node.kind === "dir" && !recursive)
            return { ok: false, err: `cannot remove '${path}': Is a directory` };
        const { dir, name } = this.parent(path, cwd);
    ...
    move(src, dst, cwd) {
        ...
        if (!node)
            return { ok: false, err: `cannot stat '${src}': No such file or directory` };
        // moving into an existing directory keeps the basename
        let target = dst;
        if (this.isDir(dst, cwd))
            target = this.abs(dst, cwd) + "/" + baseName(src);
        const to = this.parent(target, cwd);
        if (!to.dir)
            return { ok: false, err: `cannot move '${src}' to '${dst}': No such file or directory` };
        const from = this.parent(src, cwd);
        from.dir?.children.delete(from.name);
        to.dir.children.set(to.name, node);
        return { ok: true };
    }
    copy(src, dst, cwd, recursive = false) {
        const node = this.lookup(src, cwd);
        if (!node)
            return { ok: false, err: `cannot stat '${src}': No such file or directory` };
        if (node.kind === "dir" && !recursive)
            return { ok: false, err: `-r not specified; omitting directory '${src}'` };
        let target = dst;
        if (this.isDir(dst, cwd))
            target = this.abs(dst, cwd) + "/" + baseName(src);
        const to = this.parent(target, cwd);
        if (!to.dir)
            return { ok: false, err: `cannot create '${dst}': No such file or directory` };
        to.dir.children.set(to.name, clone(node));
        return { ok: true };
    }
}
export const baseName = (p) => p.replace(/\/+$/, "").split("/").pop() || "/";
export const dirName = (p) => { const s = p.replace(/\/+$/, "").split("/"); s.pop(); return s.join("/") || "/"; };
function clone(node) {
    if (node.kind === "file")
        return { ...node };
    const d = mkDir(node.owner, node.mode);
    for (const [k, v] of node.children)
        d.children.set(k, clone(v));
    return d;
}
// ---- a plausible, lived-in Debian home ----
function seed(vfs) {
    const add = (path, content, owner = "guest") => { vfs.writeFile(path, "/", content, owner); };
