...
import { VFS, baseName } from "./vfs.js";
const ok = (out = "") => ({ out, exit: 0 });
const fail = (msg, code = 1) => ({ out: msg + "\n", exit: code });
const home = (st) => (st.user === "root" ? "/root" : "/home/guest");
const short = (p, st) => (p === home(st) ? "~" : p.startsWith(home(st) + "/") ? "~" + p.slice(home(st).length) : p);
const nonFlags = (args) => args.filter((a) => !a.startsWith("-"));
const hasFlag = (args, ...fs) => args.some((a) => a.startsWith("-") && !a.startsWith("--") && fs.some((f) => a.includes(f)) || fs.includes(a));
function modeStr(node) {
    const t = node.kind === "dir" ? "d" : node.link ? "l" : "-";
    const m = node.mode, b = (v, c) => (m & v ? c : "-");
    return t + b(0o400, "r") + b(0o200, "w") + b(0o100, "x") + b(0o040, "r") + b(0o020, "w") + b(0o010, "x") + b(0o004, "r") + b(0o002, "w") + b(0o001, "x");
}
const DATE = "Jul  9 17:30";
const sizeOf = (n) => (n.kind === "dir" ? 4096 : n.content.length);
function longRow(name, node) {
...
export const CORE = {
    ...
    uname: (a) => {
        if (a.includes("-a"))
            return ok("Linux bity 6.12.74+deb13+1-cloud-arm64 #1 SMP Debian 6.12.74-2 aarch64 GNU/Linux\n");
        if (a.includes("-r"))
            return ok("6.12.74+deb13+1-cloud-arm64\n");
        if (a.includes("-m"))
            return ok("aarch64\n");
        return ok("Linux\n");
    },
    ...
    ls: (a, st) => {
        ...
        for (const p of paths) {
            ...
            if (!node) {
                out.push(`ls: cannot access '${p}': No such file or directory`);
                continue;
            }
            if (node.kind === "file") {
                out.push(long ? longRow(baseName(p), node) : p);
                continue;
            }
    ...
    cat: (a, st, stdin) => {
        ...
        for (const f of files) {
            ...
            if (!node) {
                ...
                out += node.content + (node.content.endsWith("\n") || node.content === "" ? "" : "\n");
        }
        return { out, exit };
    },
    cd: (a, st) => {
        ...
        if (!node)
            return fail(`bash: cd: ${t}: No such file or directory`);
        if (node.kind !== "dir")
            return fail(`bash: cd: ${t}: Not a directory`);
    ...
    mkdir: (a, st) => {
        const p = hasFlag(a, "p");
        for (const d of nonFlags(a)) {
            const r = st.vfs.mkdir(d, st.cwd, p, st.user);
            if (!r.ok && !p)
                return fail(`mkdir: ${r.err}`);
        }
        return ok();
    },
    rmdir: (a, st) => { for (const d of nonFlags(a)) {
        const n = st.vfs.lookup(d, st.cwd);
        if (n?.kind === "dir" && n.children.size)
            return fail(`rmdir: failed to remove '${d}': Directory not empty`);
        st.vfs.remove(d, st.cwd, true);
    } return ok(); },
    touch: (a, st) => { for (const f of nonFlags(a))
        st.vfs.touch(f, st.cwd, st.user); return ok(); },
    rm: (a, st) => {
        const rec = hasFlag(a, "r", "R"), force = hasFlag(a, "f");
        for (const f of nonFlags(a)) {
            if (f === "/")
                return fail("rm: it is dangerous to operate recursively on '/'\nrm: use --no-preserve-root to override this failsafe");
            const r = st.vfs.remove(f, st.cwd, rec);
            if (!r.ok && !force)
                return fail(`rm: ${r.err}`);
        }
        return ok();
    },
    mv: (a, st) => { const t = nonFlags(a); const dst = t.pop(); for (const s of t) {
        const r = st.vfs.move(s, dst, st.cwd);
        if (!r.ok)
            return fail(`mv: ${r.err}`);
    } return ok(); },
    cp: (a, st) => { const rec = hasFlag(a, "r", "R"); const t = nonFlags(a); const dst = t.pop(); for (const s of t) {
        const r = st.vfs.copy(s, dst, st.cwd, rec);
        if (!r.ok)
            return fail(`cp: ${r.err}`);
    } return ok(); },
    chmod: (a, st) => { for (const f of nonFlags(a).slice(1))
        if (!st.vfs.exists(f, st.cwd))
            return fail(`chmod: cannot access '${f}': No such file or directory`); return ok(); },
    wc: (a, st, stdin) => {
        ...
        if (!files.length)
            return ok(count(stdin, "") + "\n");
        let out = "";
        for (const f of files) {
            const n = st.vfs.lookup(f, st.cwd);
            if (!n || n.kind === "dir") {
                out += `wc: ${f}: ${n ? "Is a directory" : "No such file or directory"}\n`;
                continue;
            }
            out += count(n.content, " " + f) + "\n";
        }
        return ok(out);
    },
    ...
    grep: (a, st, stdin) => {
        ...
        for (const f of files) {
            const n = st.vfs.lookup(f, st.cwd);
            if (n?.kind === "file") {
                const m = scan(n.content);
                if (m.length)
                    hit = true;
                out += m.map((l) => (files.length > 1 ? `${f}:${l}` : l)).join("\n") + (m.length ? "\n" : "");
            }
        }
        return { out, exit: hit ? 0 : 1 };
    },
    sort: (a, st, stdin) => { const lines = stdin.replace(/\n$/, "").split("\n"); lines.sort(); if (hasFlag(a, "r"))
        lines.reverse(); return ok(lines.join("\n") + "\n"); },
    uniq: (a, st, stdin) => { const lines = stdin.replace(/\n$/, "").split("\n"); const out = []; let prev = null, cnt = 0; for (const l of lines) {
        if (l === prev)
            cnt++;
        else {
            if (prev !== null)
                out.push(hasFlag(a, "c") ? `${String(cnt).padStart(7)} ${prev}` : prev);
            prev = l;
            cnt = 1;
        }
    } if (prev !== null)
        out.push(hasFlag(a, "c") ? `${String(cnt).padStart(7)} ${prev}` : prev); return ok(out.join("\n") + "\n"); },
    rev: (_a, _st, stdin) => ok(stdin.replace(/\n$/, "").split("\n").map((l) => [...l].reverse().join("")).join("\n") + "\n"),
    tr: (a, st, stdin) => { const [from, to] = nonFlags(a); if (from === "a-z" && to === "A-Z")
        return ok(stdin.toUpperCase()); if (from === "A-Z" && to === "a-z")
        return ok(stdin.toLowerCase()); return ok(stdin); },
    nl: (_a, _st, stdin) => ok(stdin.replace(/\n$/, "").split("\n").map((l, i) => `${String(i + 1).padStart(6)}\t${l}`).join("\n") + "\n"),
    seq: (a) => { const n = nonFlags(a).map(Number); const [s, e] = n.length === 1 ? [1, n[0]] : [n[0], n[1]]; const out = []; for (let i = s; i <= e; i++)
        out.push(i); return ok(out.join("\n") + "\n"); },
};
function headTail(a, st, stdin, isHead) {
    const ni = a.findIndex((x) => x === "-n");
    let n = 10;
    if (ni >= 0 && a[ni + 1])
        n = parseInt(a[ni + 1], 10);
    else {
        const m = a.find((x) => /^-\d+$/.test(x));
        if (m)
            n = parseInt(m.slice(1), 10);
    }
    const files = nonFlags(a).filter((x) => !/^\d+$/.test(x));
    const slice = (text) => { const lines = text.replace(/\n$/, "").split("\n"); return (isHead ? lines.slice(0, n) : lines.slice(-n)).join("\n"); };
    if (!files.length)
        return ok(slice(stdin) + "\n");
    let out = "";
    for (const f of files) {
        const node = st.vfs.lookup(f, st.cwd);
        if (!node || node.kind === "dir") {
            out += `${isHead ? "head" : "tail"}: cannot open '${f}' for reading: No such file or directory\n`;
            continue;
        }
        out += slice(node.content) + "\n";
    }
    return ok(out);
}
