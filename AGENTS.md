...
| `corpus/generators/` | synthetic generators — **only the dreamed families are wired in** (`index.mjs`): `net.mjs`, `git.mjs`, `fun.mjs`, `unknown.mjs` (graceful `command not found`), and `sys.mjs`'s `sysinfoGen` (df/free/ps/top/lscpu/lsblk/mount/vmstat — DREAMED but capture-thin, so it needs positive signal). `sys.mjs`'s identity `sysGen` + `fs.mjs`/`copy.mjs` are **retired** (the CORE owns FS/text/identity now) — kept in-tree, unimported. `lib.mjs` = shared helpers |
